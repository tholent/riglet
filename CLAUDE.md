...
- **Control WS echoes state immediately**: after each set command (freq/mode/ptt/nudge) the `ws_control` handler sends back `{"type": "state", "<field>": <value>}` so the UI updates without waiting for the polling loop.
- **Dev-mode config path**: `_DEFAULT_CONFIG_PATH` in `config.py` respects the `RIGLET_CONFIG` env var (set by systemd on Pi), falling back to `~/.config/riglet/config.yaml` on dev. `POST /api/config/restart` reloads the manager in-place when systemctl is unavailable.
