...
All commands run from `server/` using `uv`:
...
- **`routers/`** — One module per resource: `system` (`/api/status`, `/api/config`, restart), `devices` (serial/audio enumeration, SSE hotplug), `cat` (freq/mode/PTT), `audio` (WebSocket PCM), `waterfall` (WebSocket FFT frames).
...
Svelte SPA. Not yet scaffolded (Phase 6). Static build output goes to `/opt/riglet/app/static/` and is served by FastAPI's `StaticFiles` mount. Caddy can be added later as a reverse proxy layer in the image build for HTTPS.

### Image build (`image/`)

`rpi-image-gen` based. Not yet scaffolded (Phase 8). Produces a flashable Raspberry Pi OS Lite image with all services pre-installed. First boot: if no radios are enabled in config, the backend serves a setup wizard at port 8080.
